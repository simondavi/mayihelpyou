...
data <- data %>%
  dplyr::mutate(exp_men = coalesce(BEKE2_KS, BEFA1_SQ002))
...
The high correlations between the support scales from the same source can lead to multicollinearity and suppression effects in structural equation models. To reduce these effects, different models were calculated for each kind of social support:
...
Model fit:
```{r}
fitMeasures(fit_ins_sup, c("chisq", "df", "pvalue", "cfi", "srmr", "rmsea"))
```
